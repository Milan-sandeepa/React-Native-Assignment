...
const SelectItemScreen = ({navigation}) => {
  ...
  const getAllAvalableItemsLoading = useSelector(
    state => state.shopState.getAllAvalableItemsLoading,
  );
  const getAllAvalableItemsSuccess = useSelector(
    state => state.shopState.getAllAvalableItemsSuccess,
  );
  const getAllAvalableItemsFailed = useSelector(
    state => state.shopState.getAllAvalableItemsFailed,
  );
  ...
  useEffect(() => {
    if (isFocused) {
      dispatch(getAllAvailableItemAction(data)); // Perform API call when screen is focused
  ...
  useEffect(() => {
    if (getAllAvalableItemsSuccess) {
      setItemData(getAllAvalableItemsSuccess?.result);
    } else if (getAllAvalableItemsFailed) {
    }
  }, [getAllAvalableItemsSuccess, getAllAvalableItemsFailed]);
  ...
  return (
    <View>
      ...
      <Loader isLoading={getAllAvalableItemsLoading} />
    </View>
  );
};
