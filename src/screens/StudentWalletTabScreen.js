...
const StudentWalletTabScreen = ({navigation}) => {
  ...
  const studentWalletLoading = useSelector(
    state => state.reportState.studentWalletLoading,
  );
  const studentWalletSuccess = useSelector(
    state => state.reportState.studentWalletSuccess,
  );
  const studentWalletFailed = useSelector(
    state => state.reportState.studentWalletFailed,
  );
  ...
  useEffect(() => {
    if (studentWalletSuccess) {
      setTableData(studentWalletSuccess.result);
    } else if (studentWalletFailed) {
      setTableData([]);
    }
  }, [studentWalletSuccess, studentWalletFailed]);
      ...
      <ScrollView>
        <View style={styles.container}>
          ...
          <View style={styles.mainTextContainer}>
            ...
            <TouchableOpacity onPress={() => navigation.jumpTo('DepositTab')}>
          ...
          <Loader isLoading={studentWalletLoading} />
        </View>
      </ScrollView>
    </>
  );
...
export default StudentWalletTabScreen;
