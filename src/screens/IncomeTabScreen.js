...
import {merchantIncomeAction} from '../redux/actions/reportAction';
import Loader from '../components/Loader';

const IncomeTabScreen = ({navigation}) => {
  ...
  const merchantIncomeLoading = useSelector(
    state => state.reportState.merchantIncomeLoading,
  );
  const merchantIncomeSuccess = useSelector(
    state => state.reportState.merchantIncomeSuccess,
  );
  const merchantIncomeFailed = useSelector(
    state => state.reportState.merchantIncomeFailed,
  );
  ...
  useEffect(() => {
    if (merchantIncomeSuccess) {
      setTableData(merchantIncomeSuccess?.result?.incomeList?.listData);
      setData(merchantIncomeSuccess?.result);
    } else if (merchantIncomeFailed) {
      setTableData([]);
    }
  }, [merchantIncomeSuccess, merchantIncomeFailed]);
  ...
  const getTableDataHandler = (str, end, number) => {
    ...
    dispatch(merchantIncomeAction(data));
  };
    ...
    <>
      ...
      <ScrollView>
        <View style={styles.container}>
            ...
            <View style={styles.priceContainer}>
              <View style={{flexDirection: 'row', alignItems: 'flex-end'}}>
                ...
                <Text style={styles.priceText}>
                  {' '}
                  {data?.totalIncome === '.00' || data?.totalIncome === null
                    ? '00.00'
                    : data?.totalIncome}
                </Text>
              </View>
              <Text style={styles.priceSubText}>Total Earnings</Text>
            </View>
          </View>
          ...
          <Loader isLoading={merchantIncomeLoading} />
        </View>
      </ScrollView>
    </>
  );
...
export default IncomeTabScreen;
