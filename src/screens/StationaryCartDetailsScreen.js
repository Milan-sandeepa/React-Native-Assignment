...
import {
  ...
  placeOrderStationaryRequestAction,
  removeFromCart,
  resetCartAction,
  resetPlaceOrderAction,
  resetPlaceOrderStationaryAction,
...
import {
  fileUploadAction,
  resetSelectFileAction,
} from '../redux/actions/fileUploadAction';

const StationaryCartDetailsScreen = ({navigation}) => {
  ...
  const stationaryPlaceOrderLoading = useSelector(
    state => state.cartState.stationaryPlaceOrderLoading,
  );
  const stationaryPlaceOrderSuccess = useSelector(
    state => state.cartState.stationaryPlaceOrderSuccess,
  );
  const stationaryPlaceOrderFailed = useSelector(
    state => state.cartState.stationaryPlaceOrderFailed,
  );
  ...
  useEffect(() => {
    if (stationaryPlaceOrderSuccess) {
      ...
      dispatch(resetPlaceOrderStationaryAction());
      dispatch(resetSelectFileAction());
    } else if (stationaryPlaceOrderFailed) {
      // let msg = `Sorry.Try Again`;
      let msg = `Sorry,Insufficient funds in your wallet.!`;
      showToast({code: -1, result: msg});
    }
    dispatch(resetPlaceOrderStationaryAction());
  }, [stationaryPlaceOrderFailed, stationaryPlaceOrderSuccess]);
  ...
  const orderHandler = () => {
    if (cartItems.some(item => item.docUploadStatus === 'YES')) {
      console.log('docUploadStatus: YES found.', userSave.email);
      // Create FormData
      ...
      selectFiles.forEach(item => {
        for (const key in item) {
          if (key !== 'quantity') {
            ...
            quantity.push(file.quantity);

            formData.append('file', fileData);
          }
        }
      });
      formData.append('merchantEmail', findShopSuccess?.result?.fkUser?.email);
      formData.append('userEmail', userSave.email);
      formData.append('chargeAmount', totalPrice);
      const cartItemsJson = JSON.stringify(cartItems);

      formData.append('cartItems', cartItemsJson);
      const qtyJson = JSON.stringify(quantity);
      formData.append('qty', qtyJson);
      formData.append('orderType', orderType);

      dispatch(placeOrderStationaryRequestAction(formData));
    } else {
      // placeOrderHandler();
      console.log('docUploadStatus: No found.');
    }
  };
  ...
  return (
    <View>
      ...
      <View style={styles.container}>
        ...
        <EmptyView style={{height: dimensions.heightLevel2}} />
        <TouchableOpacity style={styles.couponButton} onPress={orderHandler}>
          <Text style={{color: colors.white}}>Place Order</Text>
        </TouchableOpacity>
        <EmptyView style={{height: dimensions.heightLevel2}} />
      </View>
      {/* </ScrollView> */}
      <Loader isLoading={placeOrderLoading || stationaryPlaceOrderLoading} />
    </View>
  );
};

export default StationaryCartDetailsScreen;
